...
export const ActionButton = ({ 
    ...
    const baseStyles = 'flex items-center justify-center transition-all duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed shadow-lg dark:drop-shadow-[3px_10px_5px_rgba(0,0,0,0.25)] active:scale-98';

    const variantStyles = {
        primary: 'bg-blue-800 hover:bg-blue-600 text-white rounded-full',
        secondary: 'bg-gray-800 hover:bg-gray-700 text-white rounded-full',
        icon: 'bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 text-red-500 shadow-2xl rounded-full',
        recording: 'relative text-white bg-[#48795e] dark:text-black/60 rounded-full',
    ...
    const sizeStyles = {
        normal: 'w-32 h-12 text-base font-medium',
        large: 'w-12 h-12 rounded-full',
        small: 'p-2 text-xs bg-white/70 dark:bg-white/10 hover:bg-white/20 text-gray-700 dark:text-white backdrop-blur rounded-full',
    };
...
// bouton d'upload en logique <label>
